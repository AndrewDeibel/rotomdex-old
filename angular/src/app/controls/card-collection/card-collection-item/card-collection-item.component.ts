...
import { Condition, ConditionGraded, GradingCompany, Icons, PrintVersion } from '@app/models';
...
export class CardCollectionItemComponent implements OnInit {
	...
	buildControls() {
		...
		for (let condition in Condition) {
			this.selectCondition.optionGroups[0].options.push(new SelectOption({
				text: Condition[condition]
			}));
		}
		for (let conditionGraded in ConditionGraded) {
			this.selectCondition.optionGroups[1].options.push(new SelectOption({
		...
		for (let gradingCompany in GradingCompany) {
			this.selectGradingCompany.optionGroups[0].options.push(new SelectOption({
		...
		for (let printVersion in PrintVersion) {
			this.selectPrintVerion.optionGroups[0].options.push(new SelectOption({
